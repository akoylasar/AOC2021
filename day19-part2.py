...
def areOverlapping(s0, s1): # s0 is set
    ...
    for rs1 in rotatedS1s:
        for b1 in rs1:
            for b0 in s0:
                translation = (b0[0] - b1[0],
                               b0[1] - b1[1],
                               b0[2] - b1[2])
                transformed = set(map(lambda p: (p[0] + translation[0],
                                                 p[1] + translation[1],
                                                 p[2] + translation[2]), rs1))
                if len(s0.intersection(transformed)) >= 12:
                    return True, transformed, translation
    return False, None, None

def solve(nput):
    scannerReportToCheck = [nput[0]]
    transformedBeacons = [set(nput[0])]
    beacons = set(nput[0])
    scanners = set()
    scanners.add((0, 0, 0))
    while scannerReportToCheck:
        toRemove = scannerReportToCheck.pop()
        sa = transformedBeacons.pop()
        for sb in nput:
            if sb == toRemove:
                continue
            overlaps, transformed, scannerLocation = areOverlapping(sa, sb)
            if overlaps and sb not in scannerReportToCheck:
                scannerReportToCheck.append(sb)
                transformedBeacons.append(transformed)
                beacons = beacons.union(transformed)
                scanners.add(scannerLocation)
        nput.remove(toRemove)
    beaconA = reduce(max, scanners)
    beaconB = reduce(min, scanners)
    print(abs(beaconA[0] - beaconB[0]) +
          abs(beaconA[1] - beaconB[1]) +
          abs(beaconA[2] - beaconB[2]))
